...
class Heap(BinaryTree):
    ...
    def insert(self, value):
        ...
        binary representation of the total number of nodes
            1. You will have to explicitly store the size of
            your heap in a variable (rather than compute it)
    ...
    def remove_min(self):
        ...
        if num_nodes == 1:
            ...
            return
        binary_str = bin(num_nodes)[3:]
        last_value = Heap._remove_bottom_right(self.root, binary_str)
    ...
    def _remove_bottom_right(node, binary_str):
        ...
        if binary_str[0] == '0':
            if len(binary_str) == 1:
                ...
                mini = Heap._remove_bottom_right(node.left, binary_str[1:])
        if binary_str[0] == '1':
            if len(binary_str) == 1:
                mini = node.right.value
                node.right = None
            else:
                mini = Heap._remove_bottom_right(node.right, binary_str[1:])
        return mini
